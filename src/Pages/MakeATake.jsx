...
const TAKE_LIMIT = 240; // change to 280 if you want
...
function Segmented({ options, value, onChange }) {
  return (
    <div className="mx-auto w-[min(320px,88vw)] rounded-full border border-white/15 bg-white/10 p-1 backdrop-blur">
...
export default function MakeATake() {
  ...
  const [mode, setMode] = useState("Take"); // Take | Review
  ...
  const onSend = () => {
    ...
    // PoC “tweet”: just simulate sending
    const payload = {
    ...
    // simple UX: clear and close
    setText("");
    setStars(0);
    nav(-1);
  };

  // keyboard offset using VisualViewport (mobile chrome/safari)
  useEffect(() => {
    const vv = window.visualViewport;
    if (!vv) return;

    const onResize = () => {
      // how much viewport is “pushed up” by keyboard
  ...
  // enforce take limit (hard stop)
  const onChangeText = (v) => {
    if (mode === "Take") {
      setText(v.slice(0, TAKE_LIMIT));
    } else {
      setText(v);
    }
  };

  // switch mode adjustments
  useEffect(() => {
    if (mode === "Take") {
      setStars(0); // rating only for review
      // also clamp if user had long text
      ...
      >
        ...
        <div className="relative px-4 pt-3">
          <button
            type="button"
            onClick={() => nav(-1)}
            className="absolute left-3 top-3 grid h-10 w-10 place-items-center rounded-full bg-white/10 text-white/90 hover:bg-white/15"
            aria-label="Close"
          >
          ...
          <button
            ...
            disabled={!canSend}
            className={[
              "absolute right-3 top-3 rounded-full px-4 py-2 text-xs font-bold shadow-sm transition",
              canSend
                ? "bg-cyan-200/95 text-slate-900 hover:bg-cyan-200"
          ...
          <div className="pb-2 pt-1 text-center text-[13px] font-semibold text-white/70">
            Create
          </div>
        </div>

        {/* content scroll */}
        <div className="max-h-[86vh] overflow-y-auto px-4 pb-28 pt-3 text-white">
          {/* mode toggle */}
          ...
          <div className="mt-4">
            <div className="relative">
              ...
              <input
                ...
                className="h-11 w-full rounded-full border border-white/15 bg-white/10 pl-10 pr-4 text-sm text-white placeholder:text-white/55 outline-none backdrop-blur transition focus:border-white/25 focus:bg-white/15"
              />
            </div>
            ...
            {/* results list (optional quick picks) */}
            {query.trim().length > 0 && (
          ...
          {/* input card */}
          <div className="mt-4 rounded-3xl border border-white/10 bg-black/20 p-4 backdrop-blur">
        ...
        <div
          className="absolute left-0 right-0 z-10 bg-[#0E2A25]/95 backdrop-blur"
          style={{
            bottom: `calc(env(safe-area-inset-bottom) + ${kbOffset}px)`,
          }}
        >
          <div className="mx-4 mb-4 rounded-[22px] border border-white/10 bg-black/25 px-5 py-4">
